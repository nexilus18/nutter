...
class NutterCLI(object):
    ...
    def run(
        ...
        try:
            logging.debug(
                """ Running tests. test_pattern: {} cluster_id: {} timeout: {}
                               notebook_params: {} junit_report: {} max_parallel_tests: {}
                               tags_report: {}  recursive:{} """.format(
                    test_pattern,
                    cluster_id,
                    notebook_params,
                    timeout,
    ...
    def _handle_reports(self, report_manager, exec_results):
        ...
        for exec_result in exec_results:
            t_result = api.to_testresults(exec_result.notebook_result.exit_output)
            if t_result is None:
                print("Warning:")
                print(
                    "\tThe output of {} is missing or the format is invalid.".format(
